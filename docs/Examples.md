...
Four runnable programs, ordered so each one adds a single idea. Every one of
them imports through the facade (`batchflow`) rather than `batchflow-core`, so
they compile against the same dependency graph a user has — an example that
